...
export const PUT = async (req: Request,
  ...
  await Gourmet.findByIdAndUpdate(id, {
  category: categoryId,
  genre,
  ...
  return NextResponse.json({message:"Gourmet updated"}, {status: 200})
}
...
export const GET = async(req:Request, {params}: {params: {id:string}}) => {
  const {id} = params
  await connectMongoDB()
  const gourmet = await Gourmet.findOne({_id: id}).populate('category')
  return NextResponse.json({gourmet}, {status: 200})
}
...
export const DELETE = async(req:Request, {params}: {params: {id:string}}) => {
  ...
  try {
    const gourmet = await Gourmet.findByIdAndDelete({_id: id})
    return NextResponse.json(gourmet)   
  } catch (error) {
    console.log(error)
